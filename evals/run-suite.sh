...
# instrument and the hard ones are the only room the score has left to improve in.
# Reporting one number over both hides which of the two moved.
# The hard false-positive probes are `fp-zor`, not `zor`: a package must not average
# "must route here" with "must route nowhere", because the two are different questions and
# one number over both is not a result for either.
set -uo pipefail
