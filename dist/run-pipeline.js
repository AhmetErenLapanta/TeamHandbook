...
var PPK_BEGIN = /PuTTY-User-Key-File-\d+:[ \t]*\S|Private-Lines:[ \t]*\d/i;
var SRI_DIGEST = /\bsha(?:256-[A-Za-z0-9+/]{43}=|384-[A-Za-z0-9+/]{64}|512-[A-Za-z0-9+/]{86}==)(?![A-Za-z0-9+/=])/g;
var IMAGE_DATA_URI = /\bdata:image\/[a-z.+-]+;base64,(?:iVBORw0KGgo|\/9j\/4|R0lGOD|UklGR)[A-Za-z0-9+/]*={0,2}/g;
function withoutExemptRuns(text) {
  return text.replace(SRI_DIGEST, " ").replace(IMAGE_DATA_URI, " ");
}
var WRAPPED_LINE_MIN = 24;
var WRAPPED_LINE_RUN = 4;
var BLOB_LINE = new RegExp(`^[A-Za-z0-9+/]{${WRAPPED_LINE_MIN},}={0,2}$`);
var LINE_PREFIX = /^\s*\d+\t|^[>|*+-][ \t]?/;
function isOneStream(lines) {
  if (lines.length < WRAPPED_LINE_RUN) return false;
  return new Set(lines.slice(0, -1).map((line) => line.length)).size === 1;
}
function scanForWrappedBody(lines) {
  let run = [];
