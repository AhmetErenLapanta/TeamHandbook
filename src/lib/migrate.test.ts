...
// The other shape, and the one K-5 measured as the one both of its other nets miss: the
...
describe("shareSelection", () => {
  ...
  it("given two servers were selected, when they are shared, then ONE request claims ONE version rather than two claiming the same one", () => {
    ...
    // The defect this guards (DENETIM-URUN-1 U1): two requests opened off the same base
